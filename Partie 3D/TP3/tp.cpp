...
void updateAnimation (){
    ...
    for (long unsigned int i = 0; i < V.size(); i++) {
        V[i] = Vec3(
            V0[i].position[0] * w0 + V1[i].position[0] * w1 + V2[i].position[0] * w2,
            V0[i].position[1] * w0 + V1[i].position[1] * w1 + V2[i].position[1] * w2,
            V0[i].position[2] * w0 + V1[i].position[2] * w1 + V2[i].position[2] * w2
        );
    }

    //Ajouter des transformation
    //Translation a mettre a jour en utilisant la variable offset
    //Vec3 translation(offsetx,offsety,offsetz); // A mettre à jour
    Vec3 translation(offsetz, 0., 0.5*cos(angle));

    for (long unsigned int i = 0; i < V.size(); i++) {
        V[i] = Vec3(
            V[i].position[0] + translation[0],
            V[i].position[1] + translation[1],
            V[i].position[2] + translation[2]
        );
    }
    ...
    for (long unsigned int i = 0; i < V.size(); i++) {
        V[i] = Vec3(
            M(0,0) * V[i].position[0] + M(0,1) * V[i].position[1] + M(0,2) * V[i].position[2],
            M(1,0) * V[i].position[0] + M(1,1) * V[i].position[1] + M(1,2) * V[i].position[2],
            M(2,0) * V[i].position[7] + M(2,1) * V[i].position[1] + M(2,2) * V[i].position[2]
        );
    }
...
void idle () {
    ...
    if (currentTime - lastTime >= 1000.0*(1/60)) {
        if (auto_anim) {
            angle = cos(currentTime/200) + sin(currentTime/3000);
            interpolant2 = (int(currentTime*100)%100)/100.;
            interpolant0 = 1. - interpolant2;
            // angle -= 0.1f;
...
void printUsage () {
    cerr << endl
         ...
         << " v: Toggle automatic animation" << endl
         ...
         << " o/l, +/- : Augmente/Diminue l’offset du maillage pour la translation sur l'axe z" << endl
         << " k/m : Augmente/Diminue l’offset du maillage pour la translation sur l'axe x" << endl
         << " O/L : Augmente/Diminue l’offset du maillage pour la translation sur l'axe y" << endl
...
void key (unsigned char keyPressed, int x, int y) {
    ...
    case 'D':
        angle += 0.1f;
        if( angle >= (float)M_PI*2.f ) angle = 0.;
        interpolant1 = std::min( interpolant1 + 0.01f, 0.0f);
        updateAnimation();
        break;
    case 'd':
        angle -= 0.1f;
