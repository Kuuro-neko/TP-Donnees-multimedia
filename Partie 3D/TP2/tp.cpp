...
private:
    void init (const std::string & vertexShaderFilename,
        ...
        shadingLocation = getUniLoc ("shading");
    }
...
void setShaderValues () {
    ...
    phongShader->setLevel (levels);
    phongShader->setShading (shading);
}
...
void printUsage () {
    cerr << endl
         ...
         << " L/l: Increase/Decrease levels of cel shading" << endl
         << " r: Reset to default values" << endl
...
void key (unsigned char keyPressed, int x, int y) {
    ...
    case 'w':
        ...
        break;
    case 'A':
        if (ambientRef >= 0.999f) ambientRef=1.0f;
        else ambientRef+=0.1f;
        break;
    case 'a':
        if (ambientRef <= 0.001f ) ambientRef=0.0f;
        else ambientRef-=0.1f;
        break;
    case 'D':
        if (diffuseRef >= 0.999f) diffuseRef=1.0f;
        else diffuseRef+=0.1f;
        break;
    case 'd':
        if (diffuseRef <= 0.001f ) diffuseRef=0.0f;
        else diffuseRef-=0.1f;
        break;
    case 'S':
        if (specularRef >= 0.999f) specularRef=1.0f;
        else specularRef+=0.1f;
        break;
    case 's':
        if (specularRef <= 0.001f ) specularRef=0.0f;
        else specularRef-=0.1f;
        break;
    case 'L':
        levels += 1.0f;
        break;
    case 'l':
        if (levels > 2.0 ) levels--;
        break;
    case '+':
        shininess += 1.0f;
        break;
    case '-':
        if (shininess <= 0.001f ) shininess=0.0f;
        else shininess-=1.0f;
        break; 
    case '?':
    default:
